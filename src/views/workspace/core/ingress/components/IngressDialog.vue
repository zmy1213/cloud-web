...
<script setup lang="ts">
  ...
  import { getIngressClassListApi } from '@/api/workload/cluster-resource'
  interface Props {
    visible: boolean
    dialogType: 'add' | 'edit'
  ...
  const availableServices = ref<ServiceListItem[]>([])
  ...
  const handleServiceVisibleChange = async (visible: boolean) => {
    if (visible && availableServices.value.length === 0 && props.workspace) {
      await loadServices()
    }
  }

  const loadServices = async () => {
    if (!props.workspace || serviceLoading.value) return
